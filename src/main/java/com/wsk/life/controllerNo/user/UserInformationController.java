...
import com.wsk.life.config.tool.FileUtils;
...
import static com.wsk.life.config.RepeatSubmit.isRepeatSubmit;
...
//            name = Tool.getInstance().txtReplace(name);
    ...
    public String modifyAvatar(@RequestParam("fileName") MultipartFile file, Map<String, Object> map,HttpServletRequest request){
        // 要上传的目标文件存放路径
        String localPath = "D://Movie//images//";
        // 上传成功或者失败的提示
        String msg = "";
        ...
        if (!Tool.getInstance().isNullOrEmpty(file)) {
