 ...
 */
public interface BookNameRepository extends JpaRepository<BookNameEntity, String>{

    //随机选择10条数据
    @Query(value = "select b.* from book b WHERE  status = 1 order by rand() limit 10 ", nativeQuery = true)
    List<BookNameEntity> randBook();
    ...
    BookNameEntity findOneBook(@Param("id") long id);
}